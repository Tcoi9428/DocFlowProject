...
def _insert_number_in_adjacent_cell(document_xml, registration_number):
    marker_position = document_xml.find(NUMBER_MARKER)
    if marker_position == -1:
        raise ValueError("В шаблоне письма не найдена ячейка с символом «№».")
    ...
    if min(marker_cell_end, number_cell_start, number_cell_end, paragraph_end) == -1:
        raise ValueError("В шаблоне письма не найдена соседняя ячейка для номера.")

    number_run = (
        '<w:r><w:rPr><w:rFonts w:ascii="Times New Roman" w:hAnsi="Times New Roman" '
        'w:cs="Times New Roman"/><w:sz w:val="24"/><w:szCs w:val="24"/></w:rPr>'
        f"<w:t>{registration_number}</w:t></w:r>"
    ).encode("utf-8")
...
def reserve_correspondence_number(user, kind):
    ...
    reserved_number = max(sequence.next_number, last_used + 1)
