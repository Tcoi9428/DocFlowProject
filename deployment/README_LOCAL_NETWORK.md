...
для повторения временно неудачных отправок. Перед обработкой очереди скрипт также создает напоминания
по задачам, до срока которых остался один день или меньше.
...
Чтобы показать информационную страницу при плановом обслуживании, добавьте в аргументы службы:

```text
-MaintenanceMode true
```

После изменения параметров перезапустите службу. Django будет отвечать кодом `503 Service Unavailable`
и показывать страницу технических работ. Для возврата в рабочий режим укажите:

```text
-MaintenanceMode false
```

Если служба DocFlow полностью остановлена, Django не может отдать даже страницу-заглушку. Для такого
сценария подготовлен автономный файл `deployment/maintenance.html`. После установки IIS или другого
reverse proxy его можно настроить как резервную страницу при недоступности Waitress на порту `8010`.

Неизвестные адреса в работающем приложении при `DEBUG=False` автоматически показывают пользовательскую
страницу `404`.
